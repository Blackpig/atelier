...
class BlockManager extends Field
{
    ...
    /**
     * Configure a block with multiple field configs or schema modifier
     *
     * Accepts either:
     * - Array of field configurations: ['fieldName' => ['config' => 'value']]
     * - Closure for full schema modification: fn($schema) => $modifiedSchema
     *
     * @param string $blockClass
     * @param array|\Closure $configuration
     * @return static
     */
    public function configureBlock(string $blockClass, array|\Closure $configuration): static
    {
        if ($configuration instanceof \Closure) {
            // Schema modifier - store with special key
            $key = $blockClass . '::__schema_modifier__';
            $this->fieldConfigurations[$key] = $configuration;
        } else {
            // Array of field configs - register each one
            foreach ($configuration as $fieldName => $config) {
                $this->configureField($blockClass, $fieldName, $config);
            }
        }
    ...
    public function getFieldConfigurations(): array
    {
        ...
        foreach ($this->fieldConfigurations as $key => $config) {
            // Skip schema modifiers - they can't be serialized to frontend
    ...
    protected function saveBlockAttributes(AtelierBlock $block, array $data, string $blockType): void
    {
        ...
        // Clean up ChambreNoir image files before deleting attributes
        // ONLY clean up images that are being replaced or removed (not recreated with same data)
        $oldAttributes = $block->attributes()->get();

        if (class_exists(\BlackpigCreatif\ChambreNoir\Services\ImageCleanupService::class)) {
            $cleanupService = app(\BlackpigCreatif\ChambreNoir\Services\ImageCleanupService::class);
            $cleanedCount = 0;

            foreach ($oldAttributes as $attribute) {
                // Check if this is a ChambreNoir image
                if (!is_string($attribute->value)) {
                    continue;
                }

                $oldImageData = json_decode($attribute->value, true);
                if (!is_array($oldImageData) || !isset($oldImageData['original'])) {
                    continue;
                }

                // Check if new data has the SAME image (by comparing 'original' path)
                $newValue = $data[$attribute->key] ?? null;
                $newImageData = is_array($newValue) ? $newValue : null;

                // If new data has the same original file, DON'T clean up
                if ($newImageData && isset($newImageData['original']) &&
                    $newImageData['original'] === $oldImageData['original']) {
                    \Log::debug('BlockManager: Preserving unchanged image', [
                        'block_id' => $block->id,
                        'key' => $attribute->key,
                        'original' => $oldImageData['original'],
                    ]);
                    continue;
                }

                // Image is being replaced or removed - clean it up
                $success = $cleanupService->cleanupSingleImage($oldImageData, 'public', [
                    'block_id' => $block->id,
                    'key' => $attribute->key,
                ]);
            ...
            if ($cleanedCount > 0) {
                \Log::debug('BlockManager: Cleaned up ChambreNoir images', [
                    'block_id' => $block->id,
                    'images_cleaned' => $cleanedCount,
                ]);
            }
        }
